...
#define TWO_MINUTES_TIMEOUT 120 // in seconds
...
volatile unsigned int long_watchdog_counter = FIVE_MINUTES_TIMEOUT;
...
void loop(void)
{
  ...
  if((long_watchdog_counter > FIVE_MINUTES_TIMEOUT)){
  ...
  if((short_watchdog_counter > TWO_MINUTES_TIMEOUT)){
